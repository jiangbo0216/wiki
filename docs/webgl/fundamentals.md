...
显然我们需要告诉WebGL要运行六次顶点着色器来画两个三角形。 所以我们将`count`改成`6`。
...
无论你是不是web开发新手都请查看[设置和安装](https://webglfundamentals.org/webgl/lessons/zh_cn/webgl-setup-and-installation.html)， 获取一些WebGL开发的小技巧。
