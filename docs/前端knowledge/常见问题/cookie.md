...
cookie是直接存储在浏览器中的一小串数据。它们是HTTP协议的一部分，由[RFC 6265](https://tools.ietf.org/html/rfc6265)规范定义.

Cookies are usually set by a web-server using the response `Set-Cookie` HTTP-header. Then, the browser automatically adds them to (almost) every request to the same domain using the `Cookie` HTTP-header.

cookie通常由web服务器使用响应“set - cookie”HTTP-header来设置。然后，浏览器使用“Cookie”HTTP-header自动将它们添加到(几乎)到同一域的每个请求中。

One of the most widespread use cases is authentication:

最普遍的一个用例是身份验证:

1. Upon sign in, the server uses the `Set-Cookie` HTTP-header in the response to set a cookie with a unique “session identifier”. 登录时，服务器在响应中使用“set - cookie”http头来设置一个具有唯一“会话标识符”的cookie。
2. Next time when the request is sent to the same domain, the browser sends the cookie over the net using the `Cookie` HTTP-header. 下次当请求被发送到相同的域时，浏览器使用' cookie ' HTTP-header通过网络发送cookie。
3. So the server knows who made the request. 这样服务器就知道是谁发出的请求。

We can also access cookies from the browser, using `document.cookie` property.

我们还可以使用“文档”(document)从浏览器访问cookie。

There are many tricky things about cookies and their options. In this chapter we’ll cover them in detail.

## [Reading from document.cookie](https://javascript.info/cookie#reading-from-document-cookie)
...
## [Writing to document.cookie](https://javascript.info/cookie#writing-to-document-cookie)
...
### [XSRF attack](https://javascript.info/cookie#xsrf-attack)
...
### [Enter cookie samesite option](https://javascript.info/cookie#enter-cookie-samesite-option)
...
## [Appendix: Cookie functions](https://javascript.info/cookie#appendix-cookie-functions)
...
## [Appendix: Third-party cookies](https://javascript.info/cookie#appendix-third-party-cookies)
...
## [Appendix: GDPR](https://javascript.info/cookie#appendix-gdpr)
...
## [Summary](https://javascript.info/cookie#summary)
