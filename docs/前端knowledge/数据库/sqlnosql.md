...
指的是由二维表结构来逻辑表达和实现的数据, 严格遵循数据格式与长度的规范, 也成为行数据

#### 非结构化数据:

数据结构不规则或不完整, 没有任何预定义的数据模型, 不方便用二维逻辑表来表现的数据

#### 关系型数据库范式的存储架构演进

1. 初期一个应用服务器 + 一个关系型数据库, 每次读写数据库
2. 规模扩大, 一台应用服务器扛不住上游过来的流量且一台应用服务器会产生单点故障, 因此加应用服务器并且在流量入口使用Nginx做一层负载均衡
3. 规模继续扩大, 此时由于读写都在同一个数据库,  数据库性能出现一定的瓶颈, 此时做一层读写分离, 每次写主库, 读备库, 主备库通过binlog同步数据.
4. 业务更加大, 做了读写分离数据库压力还是越来越大, 一台数据库扛不住, 分好几台, 做分库分表

#### 关系型数据库优点

1. 容易理解(贴近逻辑世界)
2. 操作方便(sql)
3. 数据一致性(ACID), 原子性(atomicity), 一致性(consistency), 隔离性(isolation), 持久性(durability)
4. 数据稳定(持久化)
5. 服务稳定
...
1. 高并发下IO压力大(数据按行存储, 即使只针对其中某一列进行计算, 也会将整行数据从存储设备中读入内存, 导致IO较高)
2. 维护索引付出代价大(多级索引, 数据的更新导致索引的更新, 降低了数据库的读写能力, 索引越多, 读写能力越差, 同时索引占用的空间也很大)
3. 为了维护数据一致性代价大(数据一致性是关系型数据库的核心, 但是维护代价很大, sql标准为事务定义了不同的隔离级别, 从低到高依次是 **读未提交**, **读已提交**, **可重复读**, **可串行**)
4. 表结构扩展不方便(schema 固定, 扩展不方便, 需要修改表结构, DDL语句修改期间会导致缩表, 部分服务不可用)
5. 全文搜索能力弱(不具备分词能力)
...
倒排索引则完全相反，它是**关键字-->文档**的映射

优点:

- 支持分词, 全文搜索
- 支持条件查询, 聚合操作, 类似Group by
- 写文件不丢失
- 高可用
...
![img](sqlnosql-imgs/801753-20190810184004790-1575824774.png)
...
![img](sqlnosql-imgs/801753-20190809095956856-2104703547.png)
...
![img](sqlnosql-imgs/801753-20190809170902746-1184778039.png)
...
![img](sqlnosql-imgs/801753-20190809171419778-2018680703.png)
...
![img](sqlnosql-imgs/801753-20190810164641874-1306125222.png)
...
![img](sqlnosql-imgs/801753-20190810184027372-913062087.png)

看到每行有name、phone、address三个字段，这是行式存储的方式，且可以观察id = 2的这条数据，即使phone字段没有，它也是占空间的。

列式存储完全是另一种方式，它是按每一列进行组织的数据：

![img](sqlnosql-imgs/801753-20190810184230524-1159685808.png)

![img](sqlnosql-imgs/801753-20190810184238996-511911560.png)
...
![img](sqlnosql-imgs/801753-20190810185504854-894939999.jpg)
...
![img](sqlnosql-imgs/801753-20190811001224342-1200752110.png)
...
![img](sqlnosql-imgs/801753-20190811002203531-672188533.png)
...
![img](sqlnosql-imgs/801753-20190810232930008-331448438.png)
...
![img](sqlnosql-imgs/801753-20190812094840816-785471233.png)
