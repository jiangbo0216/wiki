...
## [#](http://hcysun.me/vue-design/zh/renderer-diff.html#减小dom操作的性能开销)减小DOM操作的性能开销

上一章我们讨论了渲染器是如何更新各种类型的 `VNode` 的，实际上，上一章所讲解的内容归属于完整的 `Diff` 算法之内，但并不包含核心的 `Diff` 算法。那什么才是核心的 `Diff` 算法呢？看下图：

![img](dom-diff-imgs/patch-children-3.06453ea2.png)
...
![img](dom-diff-imgs/diff-1.284cea98.png)
...
![img](dom-diff-imgs/diff-2.8ce72d46.png)
...
![img](dom-diff-imgs/diff-3.1d1958ef.png)
...
![img](dom-diff-imgs/diff-4.0adb55a1.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/qqxxlxzwm6

实际上，这个算法就是在没有 `key` 时所采用的算法，该算法是存在优化空间的，下面我们将分析如何进一步优化。

## [#](http://hcysun.me/vue-design/zh/renderer-diff.html#尽可能的复用-dom-元素)尽可能的复用 DOM 元素

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#key-的作用)key 的作用
...
![img](dom-diff-imgs/diff-5.ba7c26fa.png)
...
![img](dom-diff-imgs/diff-6.1e914878.png)
...
### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#找到需要移动的节点)找到需要移动的节点
...
![img](dom-diff-imgs/diff-react-1.7b07877f.png)
...
![img](dom-diff-imgs/diff-react-2.e6cef98d.png)
...
### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#移动节点)移动节点
...
![img](dom-diff-imgs/diff-react-3.dbc9ff62.png)
...
![img](dom-diff-imgs/diff-react-4.7443f559.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/4x6qo5w34w

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#添加新元素)添加新元素

在上面的讲解中，我们一直忽略了一个问题，即新 `children` 中可能包含那些不能够通过移动来完成更新的节点，例如新 `children` 中包含了一个全新的节点，这意味着在旧 `children` 中是找不到该节点的，如下图所示：

![img](dom-diff-imgs/diff-react-5.d12b2ed9.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/54215km3vn

TIP

实际上，所有与挂载和 `patch` 相关的函数都应该接收 `refNode` 作为参数，这里我们旨在让读者掌握核心思路，避免讲解过程的冗杂。

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#移除不存在的元素)移除不存在的元素

除了要将全新的节点添加到容器元素之外，我们还应该把已经不存在了的节点移除，如下图所示：

![img](dom-diff-imgs/diff-react-6.4ad1a4c1.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/844lp3mq72

至此，第一个完整的 `Diff` 算法我们就讲解完毕了，这个算法就是 `React` 所采用的 `Diff` 算法。但该算法仍然存在可优化的空间，我们将在下一小节继续讨论。

## [#](http://hcysun.me/vue-design/zh/renderer-diff.html#另一个思路-双端比较)另一个思路 - 双端比较

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#双端比较的原理)双端比较的原理

刚刚提到了 `React` 的 `Diff` 算法是存在优化空间的，想要要找到优化的关键点，我们首先要知道它存在什么问题。来看下图：

![img](dom-diff-imgs/diff-vue2-1.216b174f.png)

在这个例子中，我们可以通过肉眼观察从而得知最优的解决方案应该是：**把 `li-c` 节点对应的真实 DOM 移动到最前面即可**，只需要一次移动即可完成更新。然而，`React` 所采用的 `Diff` 算法在更新如上案例的时候，会进行两次移动：

![img](dom-diff-imgs/diff-vue2-2.65a1bd51.png)
...
![img](dom-diff-imgs/diff-vue2-3.933b8708.png)
...
![img](dom-diff-imgs/diff-vue2-4.b1c3cc2a.png)
...
![img](dom-diff-imgs/diff-vue2-5.0c42d714.png)

由于 `li-d` 节点所对应的真实 DOM 元素已经更新完成且被移动，所以现在真实 DOM 的顺序是：`li-d`、`li-a`、`li-b`、`li-c`，如下图所示：

![img](dom-diff-imgs/diff-vue2-6.80733f9c.png)
...
![img](dom-diff-imgs/diff-vue2-7.6474272b.png)
...
![img](dom-diff-imgs/diff-vue2-8.562f3899.png)
...
![img](dom-diff-imgs/diff-vue2-9.532508e2.png)

另外，观察上图可以发现，此时 `oldStartIdx` 和 `newStartIdx` 分别比 `oldEndIdx` 和 `newEndIdx` 要大，所以这将是最后一轮的比对，循环将终止，以上就是双端比较的核心原理。

TIP

完整代码&在线体验地址：https://codesandbox.io/s/xvmqn58jqw

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#双端比较的优势)双端比较的优势
...
![img](dom-diff-imgs/diff-vue2-10.2a5c7497.png)
...
![img](dom-diff-imgs/diff-vue2-13.b88d7abb.png)
...
![img](dom-diff-imgs/diff-vue2-14.ca021a85.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/vjp265qxnl

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#添加新元素-2)添加新元素

在上一小节中，我们尝试拿着新 `children` 中的第一个节点去旧 `children` 中寻找与之拥有相同 `key` 值的可复用节点，然后并非总是能够找得到，当新的 `children` 中拥有全新的节点时，就会出现找不到的情况，如下图所示：

![img](dom-diff-imgs/diff-vue2-15.08ec9ab8.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/n7y46ojv4m

但这么做真的就完美了吗？不是的，来看下面这个例子，我们更换新 `children` 中节点的顺序，如下图所示：

![img](dom-diff-imgs/diff-vue2-16.ce5c6d2b.png)
...
![img](dom-diff-imgs/diff-vue2-17.b4d9f7a4.png)

接着，开始下一轮的比较，重新从第一步开始。结果和上一轮相似，同样在第二步中找到可复用的节点，所以在在这一轮的更新完成之后，其状态如下图所示：

![img](dom-diff-imgs/diff-vue2-18.cc8a1fae.png)
...
![img](dom-diff-imgs/diff-vue2-19.1d4087f6.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/ryryx6n42m

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#移除不存在的元素-2)移除不存在的元素

对于双端比较，最后一个需要考虑的情况是：当有元素被移除时的情况，如下图所示：

![img](dom-diff-imgs/diff-vue2-20.a80cbba0.png)
...
![img](dom-diff-imgs/diff-vue2-21.f4b83431.png)
...
![img](dom-diff-imgs/diff-vue2-22.cb427ed4.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/9jnvjj1mko
...
## [#](http://hcysun.me/vue-design/zh/renderer-diff.html#inferno-所采用的核心-diff-算法及原理)inferno 所采用的核心 Diff 算法及原理

在 `Vue3` 中将采用另外一种核心 `Diff` 算法，它借鉴于 [ivi](https://github.com/localvoid/ivi) 和 [inferno](https://github.com/infernojs/inferno)，看下图：

![img](dom-diff-imgs/diff-benchmark.6d4927ac.png)
...
### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#相同的前置和后置元素)相同的前置和后置元素
...
![img](dom-diff-imgs/diff1.7cd0d7d1.png)
...
![img](dom-diff-imgs/diff2.469b3f9b.png)
...
![img](dom-diff-imgs/diff3.58e3c241.png)
...
![img](dom-diff-imgs/diff4.b6517d9f.png)
...
![img](dom-diff-imgs/diff5.edd80c32.png)
...
![img](dom-diff-imgs/diff6.a882df3d.png)
...
![img](dom-diff-imgs/diff7.df9450ee.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/5yo3z824vp

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#判断是否需要进行-dom-移动)判断是否需要进行 DOM 移动
...
![img](dom-diff-imgs/diff8.f618edee.png)
...
![img](dom-diff-imgs/diff9.c4746f37.png)
...
![img](dom-diff-imgs/diff10.f2a5a3e6.png)
...
![img](dom-diff-imgs/diff11.48afbeb3.png)
...
![img](dom-diff-imgs/diff12.566f24a9.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/03o5plkv40

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#dom-移动的方式)DOM 移动的方式
...
![img](dom-diff-imgs/diff14.7c7a1bfc.png)
...
![img](dom-diff-imgs/diff15.087a1726.png)
...
![img](dom-diff-imgs/diff16.3bb06229.png)
...
![img](dom-diff-imgs/diff17.21e18aa5.png)
...
![img](dom-diff-imgs/diff18.6f45cb0f.png)

`li-c` 节点既不是新节点，也不需要被移动，至此循环结束，更新完成。

TIP

完整代码&在线体验地址：https://codesandbox.io/s/4lrqpv0jm9

### [#](http://hcysun.me/vue-design/zh/renderer-diff.html#求解最长递增子序列)求解最长递增子序列
...
![img](dom-diff-imgs/lis1.a175cde6.png)

我们为原序列中的每个数字分配一个格子，并且这些格子填充 `1` 作为初始值：

![img](dom-diff-imgs/lis2.af71d744.png)

根据前面的分析，我们分别求得子问题的序列 `[ 10 ]` 和 `[ 2, 10 ]` 的最长递增子序列的长度分别为 `1` 和 `2`，所以我们修改对应的格子中的值，如下：

![img](dom-diff-imgs/lis3.78ad38df.png)
...
![img](dom-diff-imgs/lis4.aa68a822.png)
...
![img](dom-diff-imgs/lis5.6891d76d.png)
...
![img](dom-diff-imgs/lis6.188e964d.png)
...
![img](dom-diff-imgs/lis7.4e6c0065.png)
...
完整代码&在线体验地址：https://codesandbox.io/s/32wjmo7omq

## [#](http://hcysun.me/vue-design/zh/renderer-diff.html#不足之处)不足之处
...
## [#](http://hcysun.me/vue-design/zh/renderer-diff.html#references)References
